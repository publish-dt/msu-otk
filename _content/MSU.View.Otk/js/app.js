...
intervalAddUser = 300000 - 15000; // (млсек.) продолжительность интервала, через которые будет периодически отправляться запрос на подтверждение активности пользователя. 300000 - длительность хэша _!UsersOnline_; 15000 - это 10 сек. (берётся из every 10s в hx-trigger) + 5 сек. запаса
